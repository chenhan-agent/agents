...
- 至少 1 個線索是 **city-system clue**：排隊、移動、轉乘、付款、購物節奏、消費 ritual
- 至少 1 個線索是 **traveler-friction clue**：走到腿痠、吃到失手、趕路、繞路、爆買、節奏失控
- 至少 1 個線索可以合理誤導到另一個觀光城市
- 不把「經典食物 + 主題樂園 / 唯一 landmark」同時放進同一組
- 任兩個 emoji 不應足以讓 mini model 直接鎖定答案
- 如果整組最後可以被濃縮成「搞笑 / 熱鬧 + 章魚燒 + 道頓堀 / 橋」，應視為太直，因為 mini model 很容易只靠 postcard 常識把理由講完整
...
另外，建議 travel 類每輪保留至少 **1 題弱點哨兵題**（例如 `大阪旅遊`）：

- 不是因為它要常出現，而是因為它很容易讓 generator 掉回「名物 + 熱鬧 + 地標」模板
- 如果這類哨兵題還是讓 mini model 能把理由講得很完整，就不應過度樂觀看待 travel 規則已有改善
